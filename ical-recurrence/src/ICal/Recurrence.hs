...
data EventOccurrence = EventOccurrence
  { eventOccurrenceStart :: !(Maybe DateTimeStart),
    eventOccurrenceEnd :: !(Either DateTimeEnd Duration)
  }
...
recurEvents limit RecurringEvent {..} =
  let -- @
      ...
      -- be one day.
      -- @
      endDuration = case recurringEventEnd of
        Nothing -> Right durationOneDay
        Just ed -> ed

      -- TODO the spec also says this, but it's not clear what how to use that yet:
      --
      -- @
      -- For cases where a "VEVENT" calendar component
      ...
      startEvent =
        EventOccurrence
          { eventOccurrenceStart = recurringEventStart,
            eventOccurrenceEnd = endDuration
          }
   in case recurringEventStart of
        Nothing -> S.singleton startEvent
        Just startDateTime ->
          let Recurrence {..} = recurringEventRecurrence
              occurrencesFromRecurrenceDateTimes = recurRecurrenceDateTimes startDateTime endDuration recurrenceRecurrenceDateTimes
              occurrencesFromRecurrenceRules = recurRecurrenceRules limit startDateTime endDuration recurrenceRecurrenceRules
              -- @
              -- The final recurrence set is generated by gathering all of the
              -- start DATE-TIME values generated by any of the specified "RRULE"
              -- and "RDATE" properties, and then excluding any start DATE-TIME
...
-- makeEndDuration :: Maybe DateTimeStart -> Maybe (Either DateTimeEnd Duration) -> Maybe (Either DateTimeEnd Duration)
-- makeEndDuration mStart mEndDuration = case (mStart, mEndDuration) of
--   (Nothing, Nothing) -> Nothing
--   (Just _, Nothing) -> Nothing
--   (Nothing,Just (Left _)) -> Nothing
--   (_,Just (Right duration)) -> Right duration
--   (Just start, Just (Left end)) ->

-- | Compute the occurrences that the recurrence date times imply
recurRecurrenceDateTimes ::
  DateTimeStart ->
  Either DateTimeEnd Duration ->
